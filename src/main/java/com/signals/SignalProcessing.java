...
public class SignalProcessing {
    ...
    public static void decode8PSK_3000(List<Character> sig, String mode) throws URISyntaxException, IOException {
       //TODO LIST
        String bw = "77";
        int start = 0;
        ...
        while (start < sig.size()) {
            ...
            dataBlock = WalsheDecoder.fastDecode(dataBlock, decodeTableWalsh, decodeTableBin, 30,
                    "16", "4"); //16- разрядность, 4 - количиство повторов
            List<String> dataBlockListString = deInterleaveEngine.runEngine(dataBlock.stream()
                    .map(String::valueOf)
                    .collect(Collectors.toList()),4, 24,
                    "Custom_decode", "FromLeftToRight", "FromUpToDown");
